...
5. **Antigravity als bevorzugten Cloud-Skriptpfad anbinden.** Nur Headless-JSON mit Schema, Sandbox, isoliertem leerem Arbeitsverzeichnis und ohne freigegebene Tools; Qwen nur als letzte Ausweichlösung. **Abgeschlossen und geprüft.**
6. **MoneyPrinterTurbo als optionalen Produktionsadapter pilotieren.** Nicht als Ersatz für Freigaben und Provenienz; erst gleicher Testauftrag gegen den bestehenden Remotion-/Stockpfad, dann Qualitätsentscheidung. **Teilweise abgeschlossen; echter Vergleichslauf offen.**
...
- **Schritt 5 abgeschlossen:** Das vollständige Profil `Cloud-Qualität · Strichmännchen` wählt Antigravity standardmäßig für Skript und Szenenplan. Die offizielle `agy`-CLI läuft headless über stdin, mit JSON-Schema, Sandbox, Plan-Modus, deaktivierten Slash-Befehlen, unterdrücktem CLI-Log und einem pro Aufruf neuen leeren Arbeitsverzeichnis. Prompts, Antworten, Sitzungskennungen und CLI-Diagnosen werden nicht protokolliert. Werkzeugversuche, Zeitüberschreitungen, Authentifizierungs-/Kapazitätsfehler und ungültige Antworten werden kategorisiert und ohne sensible Rohdaten angezeigt.
- **Provenienz und Ausfallsicherheit Schritt 5:** Tatsächlich verwendeter Anbieter und Modell werden am Auftrag sowie in den Metadaten jeder Skriptversion gespeichert und angezeigt. Ein erlaubter Rückfall auf Qwen speichert den angeforderten Anbieter und einen kategorisierten Grund und wird in der Weboberfläche als Anbieterwechsel hervorgehoben. Stabile Auftrags-IDs und ein In-Flight-Deduplikator verhindern doppelte Aufträge beziehungsweise parallele identische CLI-Läufe; ein Fehler vor Schemaerfolg hinterlässt keinen Teilauftrag.
- **Echttest Schritt 5:** Mit deaktiviertem Qwen-Fallback wurde über die Weboberfläche ein neues 30-Sekunden-Skript mit `antigravity / gemini-3.1-pro-high` erzeugt und anschließend per Änderungswunsch über denselben Anbieter auf Version 2 überarbeitet. Beide Versionen bestanden das bestehende Schema, erhielten verschiedene Inhaltshashes und gespeicherte Provenienz. Nach Serverneustart blieb Version 2 erhalten und wurde korrekt angezeigt; Browserkonsole ohne Fehler oder Warnungen. Isolierte Testdaten wurden anschließend entfernt.
- **Prüfung Schritt 5:** 93/93 Python-Tests, Python-Compileall, `node --check web/app.js`, TypeScript-`tsc --noEmit`, `git diff --check` und `npm audit --omit=dev` bestanden; npm meldet 0 bekannte Produktionsabhängigkeits-Schwachstellen. Keine neue Abhängigkeit, kein Schlüssel, keine Sitzungsdatei, keine Modelldatei und kein Testmedium wurde ins Repository aufgenommen.
- **Schritt 6 – implementierter Teil:** Das Pilotprofil ändert ausschließlich den Schnitt; Antigravity-Skript, Pexels-Medien und Piper-Stimme bleiben gleich. Ein standardmäßig abgeschalteter Adapter verwendet die offizielle Batch-CLI ohne Webserver oder Port, eine feste externe Installation samt eigener Python-Umgebung und eine bereinigte Umgebung ohne Anbieter-Schlüssel. Aufgabenkennung, genau eine Ausgabe und deren Pfad innerhalb des zugehörigen MoneyPrinterTurbo-Auftragsverzeichnisses werden geprüft. Gespeichert werden Hashes von Skript, Eingabemanifest, CLI und Ausgabe, aber weder Rohdiagnosen noch Zugangsdaten. Veröffentlichungen werden nicht angefordert.
- **Vergleich und sichtbare Grenzen Schritt 6:** Nur ein vorhandener Render mit exakt demselben Skript-Hash und dem jeweils anderen Schnittanbieter gilt als Basis. Die Weboberfläche zeigt beide Videos sowie Laufzeit, Größe, Codecs und Untertitelstatus nebeneinander. Die öffentliche Batch-CLI kann die geprüfte SRT-Datei nicht übernehmen; der Pilot erzeugt deshalb bewusst keine eigenen, unfreigegebenen Untertitel und zeigt diese Qualitätsgrenze als Warnung. Technische Werte ersetzen ausdrücklich nicht die redaktionelle Prüfung von Bildpassung, Stimme, sichtbaren Fehlern und Nachbearbeitungsaufwand.
- **Sicherheitsgrenze Schritt 6:** Der Fremdprozess erhält eine minimale Umgebung, keine Gemini-, OpenAI- oder Pexels-Schlüssel und keine vertrauliche Eingabe in der Kommandozeile. Laufzeit, stdout-Größe und Ausgabepfad sind begrenzt; bei Zeitüberschreitung wird der Prozessbaum beendet. Dies ist keine Betriebssystem-Sandbox: Fremdcode besitzt weiterhin die Rechte des lokalen Windows-Benutzers. Daher installiert oder startet die Anwendung MoneyPrinterTurbo nicht selbst.
- **Lokaler Nachweis und offener Punkt Schritt 6:** Auf diesem Rechner sind MoneyPrinterTurbo, Docker und `uv` nicht vorhanden; außerdem fehlt `PEXELS_API_KEY`. Daher ist das Profil in der Oberfläche korrekt sichtbar, aber mit konkreten Gründen deaktiviert. Es wurde kein Fremdcode installiert oder ausgeführt und kein Echttest vorgetäuscht. Die Adapter-/Vertragstests verwenden eine kontrollierte Test-CLI. Ein Browser-Smoke-Test bestätigte das deaktivierte Profil und den deaktivierten Editor ohne Konsolenfehler.
- **Teilprüfung Schritt 6:** 103/103 Python-Tests, Python-Compileall, `node --check web/app.js`, TypeScript-`tsc --noEmit`, `git diff --check` und `npm audit --omit=dev` bestanden; npm meldet 0 bekannte Produktionsabhängigkeits-Schwachstellen. Neue Tests prüfen unter anderem die abgeschaltete Voreinstellung, externe Laufzeitgrenze, bereinigte Umgebung, begrenzte Prozessausgabe, unveränderte Eingaben, Pfadvalidierung und den ausschließlich hashgleichen Vergleich.
- **Fortsetzungspunkt:** Schritt 6 bleibt bis zum echten Vergleich offen. Benötigt werden eine separat geprüfte MoneyPrinterTurbo-Installation außerhalb dieses Repositories, deren eigene Python-Umgebung und ein bewusst konfigurierter Pexels-Schlüssel. Danach ist derselbe freigegebene Auftrag zuerst mit Remotion und anschließend – bei identischem Skript-Hash – mit MoneyPrinterTurbo zu rendern und redaktionell zu vergleichen. Schritt 7 beginnt vorher nicht.
...
Für den qualitätsorientierten Skriptpfad ist Option A entschieden und Schritt 5 abgeschlossen. In Schritt 6 sind Adapter, Sicherheitsgrenzen, Auswahl und technische Vergleichsansicht implementiert; offen ist der reale Vergleichslauf mit einer separat geprüften MoneyPrinterTurbo-Installation und Pexels-Konfiguration. Der nicht mehr unterstützte Gemini-CLI-Consumerpfad bleibt deaktiviert; Antigravity ist der bevorzugte Cloud-Skriptpfad und Qwen der sichtbare letzte Fallback. Schritt 7 beginnt erst nach dem echten Qualitätsvergleich.
